...
def run_train():
    ...
    files=[]
    delf_net_ = model_h5.Model(state="test")
    names = os.listdir(args.save_ckpt)
    print(args.save_ckpt)
    print(names)
    for name in names:
        path = os.path.abspath(os.path.join(args.save_ckpt, name))
        if os.path.isfile(path):
            if "checkpoint_delf_"+args.train_state not in name:
                continue
            if 'meta' in name:
                continue
            else:
                files.append(path)
    if len(files) > 1:
        print("plese makde sure there is no ckpt file before train in "+args.save_ckpt)
        exit()
    param_dict = load_checkpoint(files[0])
    load_param_into_net(delf_net_, param_dict)

    input_batch = Tensor(np.random.uniform(
        -1.0, 1.0, size=(7, 3, 2048, 2048)).astype(np.float32))

    export(delf_net, input_batch, file_name='DELF_AIR', file_format='AIR')

    pwd = os.getcwd()
    files_=[]
    names_ = os.listdir(pwd)
    for name_ in names_:
        path = os.path.abspath(os.path.join(pwd, name_))
        if "DELF_AIR" not in name_:
            continue
        else:
            files_.append(path)
    print(files_)
    shutil.copy(os.path.join(pwd,files_[0]), '/cache/train')
